...
int main (int argc,char *argv[])
  ...
  if (config.joystick)
  {
          numofjoysticks = SDL_IsGameController(numofjoysticks);
          for (count = 0; count < numofjoysticks; count++)
          {
              joy[count] = SDL_GameControllerOpen(count);
              temp = SDL_GameControllerName(joy[count]);
              printf("Using game controller: %s\n", SDL_GameControllerName(joy[count]));
              strcpy(joystick[count].name, temp);
              joystick[count].numofbuttons = SDL_CONTROLLER_BUTTON_MAX;
              joystick[count].numofhats = SDL_CONTROLLER_AXIS_MAX;
          }
  ...
  loadtexturetga(1000, "font00.png", 0, GL_CLAMP, GL_CLAMP, GL_LINEAR, GL_LINEAR);
  loadtexturetga(1001, "font01.png", 0, GL_CLAMP, GL_CLAMP, GL_LINEAR, GL_LINEAR);

  loadtexturetga(768, "mouse00.png", 0, GL_CLAMP, GL_CLAMP, GL_LINEAR, GL_LINEAR);
  loadtexturetga(769, "mouse00.png", 0, GL_CLAMP, GL_CLAMP, GL_LINEAR, GL_LINEAR);
