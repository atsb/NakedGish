...
char* userpath(char* result, char* subdir, char* file)
{
    ...
    if (snprintf(result, MAX_PATH, USERPATH(""), env) < 0)
        goto fail;
    ...
    if (subdir)
    {
        if (snprintf(result, MAX_PATH, USERPATH("/%s"), env, subdir) < 0)
            goto fail;
        ...
        if (snprintf(result, MAX_PATH, USERPATH("/%s/%s"), env, subdir, file) < 0)
            goto fail;
        else
            return result;
    }
    ...
    {
        if (snprintf(result, MAX_PATH, USERPATH("/%s"), env, file) < 0)
            goto fail;
        else
            return result;
    }
...
fail:
    ...
    if (file)
        strncpy(result, file, MAX_PATH);
    else
        strncpy(result, ".", MAX_PATH);

    result[MAX_PATH - 1] = '\0'; /* Safety first! */
    return result;
}
