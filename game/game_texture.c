...
void loadleveltiles(char *filename)
  ...
  if ((fp=fopen(filename,"rb"))!=NULL)
    ...
    if (version==9)
      {
      fread(cryptdata,1,32,fp);
      fread(cryptdata,4,1,fp);
      fread(cryptdata,4,1,fp);
      fread(cryptdata,4,1,fp);
      fread(cryptdata,4,64*4,fp);

      fread(cryptdata,1,256*256,fp);

      fread(cryptdata,1,256*256,fp);

      fread(cryptdata,1,256*256,fp);

      fread(cryptdata,4,3,fp);
      fread(cryptdata,4,12,fp);
