...
void savelevel(char *filename)
  ...
  */
  ...
  if ((fp = fopen(userpath(path, "level", filename), "wb")) != NULL)
    ...
    fwrite(level.background,1,32,fp);
    fwrite(&level.tileset,4,1,fp);
    fwrite(&level.gametype,4,1,fp);
    fwrite(&level.time,4,1,fp);
    fwrite(level.area,4,64*4,fp);

    fwrite(level.backgrid,1,256*256,fp);

    fwrite(level.grid,1,256*256,fp);

    fwrite(level.foregrid,1,256*256,fp);

    fwrite(level.startposition,4,3,fp);
    fwrite(level.ambient,4,12,fp);
    fwrite(&level.numofobjects,4,1,fp);
    for (count=0;count<level.numofobjects;count++)
      {
      fwrite(&level.object[count].type,4,1,fp);
      fwrite(&level.object[count].texturenum,4,1,fp);
      fwrite(&level.object[count].link,4,1,fp);
      fwrite(level.object[count].position,4,3,fp);
      fwrite(&level.object[count].angle,4,1,fp);
      fwrite(level.object[count].size,4,2,fp);
      fwrite(&level.object[count].mass,4,1,fp);
      fwrite(&level.object[count].friction,4,1,fp);
      fwrite(&level.object[count].lighttype,4,1,fp);
      fwrite(level.object[count].lightcolor,4,3,fp);
      fwrite(&level.object[count].lightintensity,4,1,fp);
      fwrite(&level.object[count].ai,4,1,fp);
      }
    fwrite(&level.numofropes,4,1,fp);
    for (count=0;count<level.numofropes;count++)
      {
      fwrite(&level.rope[count].type,4,1,fp);
      fwrite(&level.rope[count].texturenum,4,1,fp);
      fwrite(&level.rope[count].obj1,4,1,fp);
      fwrite(&level.rope[count].obj1part,4,1,fp);
      fwrite(&level.rope[count].obj2,4,1,fp);
      fwrite(&level.rope[count].obj2part,4,1,fp);
      }
    ...
    for (count=1;count<251;count++)
      ...
      if (textureused[count])
        {
        fwrite(&texture[count].sizex,4,1,fp);
        if (texture[count].sizex!=0)
          {
          fwrite(&texture[count].sizey,4,1,fp);
          fwrite(&texture[count].magfilter,4,1,fp);
          fwrite(&texture[count].minfilter,4,1,fp);
          fwrite(texture[count].rgba[0],4,texture[count].sizex*texture[count].sizey,fp);
          }
        }
      else
        {
      ...
      fwrite(&block[count].numoflines,4,1,fp);
      for (count2=0;count2<block[count].numoflines;count2++)
        fwrite(block[count].line[count2],4,8,fp);
      fwrite(&block[count].friction,4,1,fp);
      fwrite(&block[count].breakpoint,4,1,fp);
      fwrite(&block[count].middamage,4,1,fp);
      fwrite(&block[count].foredamage,4,1,fp);
      fwrite(&block[count].density,4,1,fp);
      fwrite(&block[count].drag,4,1,fp);
      fwrite(&block[count].animation,4,1,fp);
      fwrite(&block[count].animationspeed,4,1,fp);
      }
...
void loadlevel(char *filename)
  ...
  if ((fp=fopen(filename,"rb"))!=NULL)
    ...
    if (version==9)
      ...
      fread(level.background,1,32,fp);
      fread(&level.tileset,4,1,fp);
      fread(&level.gametype,4,1,fp);
      fread(&level.time,4,1,fp);
      fread(level.area,4,64*4,fp);

      fread(level.backgrid,1,256*256,fp);
      memcpy(cryptdata,level.backgrid,256*256);
      decryptdata(x,256*256/4);
      memcpy(level.backgrid,cryptdata,256*256);

      fread(level.grid,1,256*256,fp);
      memcpy(cryptdata,level.grid,256*256);
      decryptdata(x,256*256/4);
      memcpy(level.grid,cryptdata,256*256);

      fread(level.foregrid,1,256*256,fp);
      memcpy(cryptdata,level.foregrid,256*256);
      decryptdata(x,256*256/4);
      memcpy(level.foregrid,cryptdata,256*256);

      fread(level.startposition,4,3,fp);
      fread(level.ambient,4,12,fp);
      fread(&level.numofobjects,4,1,fp);
      ...
      for (count=0;count<level.numofobjects;count++)
        {
        fread(&level.object[count].type,4,1,fp);
        fread(&level.object[count].texturenum,4,1,fp);
        fread(&level.object[count].link,4,1,fp);
        fread(level.object[count].position,4,3,fp);
        fread(&level.object[count].angle,4,1,fp);
        fread(level.object[count].size,4,2,fp);
        fread(&level.object[count].mass,4,1,fp);
        fread(&level.object[count].friction,4,1,fp);
        fread(&level.object[count].lighttype,4,1,fp);
        fread(level.object[count].lightcolor,4,3,fp);
        fread(&level.object[count].lightintensity,4,1,fp);
        fread(&level.object[count].ai,4,1,fp);
        }
      fread(&level.numofropes,4,1,fp);
      ...
      for (count=0;count<level.numofropes;count++)
        {
        fread(&level.rope[count].type,4,1,fp);
        fread(&level.rope[count].texturenum,4,1,fp);
        fread(&level.rope[count].obj1,4,1,fp);
        fread(&level.rope[count].obj1part,4,1,fp);
        fread(&level.rope[count].obj2,4,1,fp);
        fread(&level.rope[count].obj2part,4,1,fp);
        }
      for (count=1;count<251;count++)
        {
        fread(&texture[count].sizex,4,1,fp);
        ...
        if (texture[count].sizex!=0)
          {
          fread(&texture[count].sizey,4,1,fp);
          fread(&texture[count].magfilter,4,1,fp);
          fread(&texture[count].minfilter,4,1,fp);
        ...
        fread(&block[count].numoflines,4,1,fp);
        ...
        for (count2=0;count2<block[count].numoflines;count2++)
          fread(block[count].line[count2],4,8,fp);
        fread(&block[count].friction,4,1,fp);
        fread(&block[count].breakpoint,4,1,fp);
        fread(&block[count].middamage,4,1,fp);
        fread(&block[count].foredamage,4,1,fp);
        fread(&block[count].density,4,1,fp);
        fread(&block[count].drag,4,1,fp);
        fread(&block[count].animation,4,1,fp);
        fread(&block[count].animationspeed,4,1,fp);
        }
      }
    if (version==10)
      {
