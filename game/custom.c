  ...
  if ((fp=fopen(filename,"rb"))!=NULL)
    ...
    if (version==1)
      ...
      for (count=0;count<8;count++)
        {
        fread(&playermappack[count].levelnum,4,1,fp);
        fread(&playermappack[count].numoflives,4,1,fp);
        fread(&playermappack[count].totalscore,4,1,fp);
        fread(&playermappack[count].difficulty,4,1,fp);
        fread(&player[count].highscore,4,1,fp);
        }
      }
    fclose(fp);
    }
...
void saveplayermappack(void)
  ...
  if ((fp=fopen(filename,"wb"))!=NULL)
    ...
    for (count=0;count<8;count++)
      {
      fwrite(&playermappack[count].levelnum,4,1,fp);
      fwrite(&playermappack[count].numoflives,4,1,fp);
      fwrite(&playermappack[count].totalscore,4,1,fp);
      fwrite(&playermappack[count].difficulty,4,1,fp);
      fwrite(&player[count].highscore,4,1,fp);
      }
    fclose(fp);
    }
...
void loadmappack(void)
  ...
  if ((fp=fopen(mappack.filename,"rb"))!=NULL)
    ...
    if (version==1)
      {
      fread(&mappack.numoflevels,4,1,fp);
      fread(mappack.name,1,32,fp);
      for (count=0;count<64;count++)
        {
        fread(mappack.level[count],1,32,fp);
        fread(mappack.levelname[count],1,32,fp);
        fread(mappack.levelmusic[count],1,32,fp);
        }
      for (count=0;count<16;count++)
        fread(mappack.ending[count],1,64,fp);
      }
...
void savemappack(void)
  ...
  if ((fp=fopen(mappack.filename,"wb"))!=NULL)
    ...
    fwrite(&mappack.numoflevels,4,1,fp);
    fwrite(mappack.name,1,32,fp);
    for (count=0;count<64;count++)
      {
      fwrite(mappack.level[count],1,32,fp);
      fwrite(mappack.levelname[count],1,32,fp);
      fwrite(mappack.levelmusic[count],1,32,fp);
      }
    for (count=0;count<16;count++)
      fwrite(mappack.ending[count],1,64,fp);
