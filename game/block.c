...
void saveblock(int blocknum)
  ...
  if ((fp = fopen(userpath(path, "block", filename), "wb")) != NULL)
    {
    fwrite(&block[blocknum].numoflines,4,1,fp);
    for (count=0;count<block[blocknum].numoflines;count++)
      fwrite(block[blocknum].line[count],4,8,fp);
...
void loadblock(int blocknum)
  ...
  if ((fp=fopen(filename,"rb"))!=NULL)
    {
    fread(&block[blocknum].numoflines,4,1,fp);
    for (count=0;count<block[blocknum].numoflines;count++)
      {
      fread(block[blocknum].line[count],4,8,fp);
