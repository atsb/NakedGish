  ...
  void savescores(void)
  {
      ...
      if ((fp = fopen(userpath(path, NULL, "gish.his"), "wb")) != NULL)
      {
          ...
          for (count = 0; count < 32; count++)
              ...
              {
                  fwrite(&highscore[count][count2].time, 4, 1, fp);
                  fwrite(highscore[count][count2].name, 1, 32, fp);
              }
          fclose(fp);
      }
  }
    ...
    if ((fp = fopen(userpath(path, NULL, "gish.his"), "rb")) != NULL)
    {
        ...
        if (version == 2)
        {
            for (count = 0; count < 32; count++)
                for (count2 = 0; count2 < 16; count2++)
                {
                    fread(&highscore[count][count2].time, 4, 1, fp);
                    fread(highscore[count][count2].name, 1, 32, fp);
                }
        }
        fclose(fp);
    }
