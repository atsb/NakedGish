...
void savereplay(int levelnum)
  ...
  if ((fp = fopen(userpath(path, "replay", filename), "wb")) != NULL)
    ...
    fwrite(&levelnum,4,1,fp);
    fwrite(&numofreplayframes,4,1,fp);
    if (numofreplayframes<65536)
    for (count=0;count<numofreplayframes;count++)
      fwrite(&replayframe[count].button,1,1,fp);
    fclose(fp);
    }
...
int loadreplay(char *filename)
  ...
  if ((fp = fopen(userpath(path, "replay", filenametemp), "rb")) != NULL)
    ...
    if (version==1)
      {
      fread(&levelnum,4,1,fp);
      fread(&numofreplayframes,4,1,fp);
      if (numofreplayframes<65536)
      for (count=0;count<numofreplayframes;count++)
        fread(&replayframe[count].button,1,1,fp);
      }
    fclose(fp);
    }
