...
void objectcycle(void)
    ...
    if (rope[count].type>=5 && rope[count].type<9)
      ...
      if (rope[count].cyclelength==0.0f)
        ...
        if (rope[count].link==-1 || object[rope[count].link].idata[0]==1)
          {
          rope[count].angle+=2.0f*M_PI/(50.0f*rope[count].cycle);
          if (rope[count].angle>2.0f*M_PI)
            rope[count].angle-=2.0f*M_PI;
          }
        }
      else
        {
        if (rope[count].link==-1)
          {
          if (rope[count].cyclecount<rope[count].cyclelength*2.0f*M_PI)
            {
            rope[count].angle+=2.0f*M_PI/(50.0f*rope[count].cycle);
            if (rope[count].angle>2.0f*M_PI)
              rope[count].angle-=2.0f*M_PI;
            rope[count].cyclecount+=2.0f*M_PI/(50.0f*rope[count].cycle);
            }
          }
        else
          {
          ...
          if (object[rope[count].link].idata[0]==1 && rope[count].cyclecount==0.0f)
            count2=1;
          if (rope[count].cyclecount>0.0f && rope[count].cyclecount<rope[count].cyclelength*2.0f*M_PI)
            count2=1;
          if (rope[count].cyclecount>rope[count].cyclelength*2.0f*M_PI+2.0f*M_PI/(50.0f*rope[count].cycle))
            count2=1;
          ...
          if (count2==1)
            ...
            if (rope[count].angle>2.0f*M_PI)
              rope[count].angle-=2.0f*M_PI;
            rope[count].cyclecount+=2.0f*M_PI/(50.0f*rope[count].cycle);
            if (rope[count].cyclecount>=rope[count].cyclelength*4.0f*M_PI)
  ...
  for (count=0;count<numofobjects;count++)
    ...
    if (object[count].type==1)
      ...
      if (spin>1.0f)
        spin=1.0f;
      if (angle<object[count].angle-M_PI)
        angle+=2.0f*M_PI;
      if (angle>object[count].angle+M_PI)
        angle-=2.0f*M_PI;
      /*
      ...
      */
      ...
      if (object[count].angle<0.0f)
        object[count].angle+=2.0f*M_PI;
      if (object[count].angle>2.0f*M_PI)
        object[count].angle-=2.0f*M_PI;
