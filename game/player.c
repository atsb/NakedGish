...
void saveplayers(void)
  ...
  if ((fp = fopen(userpath(path, NULL, "gish.pla"), "wb")) != NULL)
    ...
    for (count=0;count<8;count++)
      {
      fwrite(player[count].name,1,32,fp);
      fwrite(&player[count].levelnum,4,1,fp);
      fwrite(&player[count].numoflives,4,1,fp);
      fwrite(&player[count].totalscore,4,1,fp);
      fwrite(&player[count].difficulty,4,1,fp);
      fwrite(&player[count].gamepassed,4,1,fp);
      fwrite(&player[count].highscore,4,1,fp);
      fwrite(player[count].unlock,4,16,fp);
      }
    fclose(fp);
    }
  }
...
void loadplayers(void)
  ...
  if ((fp = fopen(userpath(path, NULL, "gish.pla"), "rb")) != NULL)
    ...
    if (version==2)
      ...
      for (count=0;count<8;count++)
        {
        fread(player[count].name,1,32,fp);
        fread(&player[count].levelnum,4,1,fp);
        fread(&player[count].numoflives,4,1,fp);
        fread(&player[count].totalscore,4,1,fp);
        player[count].difficulty=1;
        fread(&player[count].gamepassed,4,1,fp);
        fread(&player[count].highscore,4,1,fp);
        fread(player[count].unlock,4,16,fp);
        }
      }
    if (version==3)
      {
      for (count=0;count<8;count++)
        ...
        fread(&player[count].numoflives,4,1,fp);
        fread(&player[count].totalscore,4,1,fp);
        fread(&player[count].difficulty,4,1,fp);
        fread(&player[count].gamepassed,4,1,fp);
        fread(&player[count].highscore,4,1,fp);
        fread(player[count].unlock,4,16,fp);
        }
